  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        <p className="text-[15px] leading-7 text-[#292929]/85">{copy.faq.intro}</p>
        <Accordion className="mt-10" defaultValue={[faqs[locale][0].q]}>
          {faqs[locale].map((item) => (
            <AccordionItem key={item.q} value={item.q} className="border-[#36348E]/15">
              <AccordionTrigger className="py-4 text-left text-base font-medium">
                {item.q}
              </AccordionTrigger>
              <AccordionContent className="text-[15px] leading-7 text-[#292929]/80">
                {item.a}
              </AccordionContent>
            </AccordionItem>
          ))}
