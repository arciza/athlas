      ...
      <section className="mx-auto grid max-w-6xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-2">
        <ol className="grid gap-4">
          {copy.processSteps.map((step) => (
            <li key={step.n} className="rounded-2xl border border-[#36348E]/10 p-5">
              <p className="text-sm font-semibold text-[#36348E]">{step.n}</p>
              <h2 className="font-heading mt-2 text-lg font-semibold">{step.title}</h2>
              <p className="mt-2 text-sm leading-6 text-[#292929]/75">{step.body}</p>
            </li>
          ))}
        </ol>
        <div className="rounded-2xl bg-[#FAFAFA] p-6 ring-1 ring-[#36348E]/10">
          <ContactForm
            locale={locale}
            variant="athlete"
