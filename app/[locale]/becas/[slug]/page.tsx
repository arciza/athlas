  ...
  return (
    <>
      ...
      <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        ...
        <p className="mt-6 text-[15px] leading-7 text-[#292929]/85">{sport.intro[locale]}</p>
        {sport.sections.map((section) => (
          <section key={section.title.es} className="mt-10">
            ...
            <p className="mt-3 text-[15px] leading-7 text-[#292929]/85">{section.body[locale]}</p>
          </section>
        ))}
        <h2 className="font-heading mt-10 text-2xl font-semibold">
          {locale === "es" ? "Conclusión" : "Conclusion"}
        </h2>
        <p className="mt-3 text-[15px] leading-7 text-[#292929]/85">{sport.conclusion[locale]}</p>
        <h2 className="font-heading mt-12 text-2xl font-semibold">{copy.sportsPage.apply}</h2>
        <p className="mt-3 leading-7 text-[#292929]/80">{copy.sportsPage.applyLead}</p>
        <Link
          href={localePath(locale, "/proceso")}
          className={cn(
            buttonVariants({ size: "lg" }),
            "mt-6 inline-flex h-11 bg-[#36348E] px-5 text-white hover:bg-[#36348E]/90"
          )}
        >
          {copy.home.start}
        </Link>
